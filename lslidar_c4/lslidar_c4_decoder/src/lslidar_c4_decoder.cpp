...
LslidarC4Decoder::LslidarC4Decoder(
    ...
    sweep_data(new lslidar_c4_msgs::LslidarC4Sweep()),
    multi_scan(new lslidar_c4_msgs::LslidarC4Layer())
    {
    return;
}

bool LslidarC4Decoder::loadParameters() {
...
bool LslidarC4Decoder::createRosIO() {
    packet_sub = nh.subscribe<lslidar_c4_msgs::LslidarC4Packet>(
                "lslidar_packet", 100, &LslidarC4Decoder::packetCallback, this);
    layer_sub = nh.subscribe(
                "layer_num", 100, &LslidarC4Decoder::layerCallback, this);
    sweep_pub = nh.advertise<lslidar_c4_msgs::LslidarC4Sweep>(
                "lslidar_sweep", 10);
    point_cloud_pub = nh.advertise<sensor_msgs::PointCloud2>(
                "lslidar_point_cloud", 10);
    scan_pub = nh.advertise<sensor_msgs::LaserScan>(
                "scan", 100);
    channel_scan_pub = nh.advertise<lslidar_c4_msgs::LslidarC4Layer>(
                "scan_channel", 100);
    return true;
}

bool LslidarC4Decoder::initialize() {
    if (!loadParameters()) {
...
bool LslidarC4Decoder::checkPacketValidity(const RawPacket* packet) {
...
void LslidarC4Decoder::publishPointCloud() {
    ...
    for (size_t i = 0; i < 16; ++i) {
        const lslidar_c4_msgs::LslidarC4Scan& scan = sweep_data->scans[i];
...
void LslidarC4Decoder::publishChannelScan()
{
    multi_scan = lslidar_c4_msgs::LslidarC4LayerPtr(
                    new lslidar_c4_msgs::LslidarC4Layer());
    // lslidar_c4_msgs::LslidarC4Layer multi_scan(new lslidar_c4_msgs::LslidarC4Layer);
...
void LslidarC4Decoder::publishScan()
{
...
point_struct LslidarC4Decoder::getMeans(std::vector<point_struct> clusters)
{
    point_struct tmp;
...
void LslidarC4Decoder::decodePacket(const RawPacket* packet) {
...
void LslidarC4Decoder::layerCallback(const std_msgs::Int8Ptr& msg){
    int num = msg->data;
    if (num < 0)
    {
        num = 0;
...
void LslidarC4Decoder::packetCallback(
        const lslidar_c4_msgs::LslidarC4PacketConstPtr& msg) {
    ...
    for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
        for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx) {
            ...
            sweep_data->scans[remapped_scan_idx].points.push_back(
                        lslidar_c4_msgs::LslidarC4Point());

            lslidar_c4_msgs::LslidarC4Point& new_point =		// new_point 为push_back最后一个的引用
    ...
    if (end_fir_idx != FIRINGS_PER_PACKET) {
        ...
        sweep_data = lslidar_c4_msgs::LslidarC4SweepPtr(
                    new lslidar_c4_msgs::LslidarC4Sweep());
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
            for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx) {
                ...
                sweep_data->scans[remapped_scan_idx].points.push_back(
                            lslidar_c4_msgs::LslidarC4Point());
                lslidar_c4_msgs::LslidarC4Point& new_point =
                        sweep_data->scans[remapped_scan_idx].points[
                        sweep_data->scans[remapped_scan_idx].points.size()-1];
